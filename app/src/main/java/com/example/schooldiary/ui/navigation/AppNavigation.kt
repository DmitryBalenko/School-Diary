...
fun AppNavigation() {
    ...
    NavHost(
        ...
        composable("edit_day/{dayKey}") { entry ->
            EditDayScreen(
                navController,
                scheduleManager,
                bellManager,
                entry.arguments?.getString("dayKey") ?: "MONDAY",
                lang
            )
        }
        ...
        composable("write_hw/{subject}/{icon}") { entry ->
            WriteHomeworkScreen(
                navController,
                hwManager,
                scheduleManager,
                settingsManager,
                Uri.decode(entry.arguments?.getString("subject") ?: ""),
                Uri.decode(entry.arguments?.getString("icon") ?: ""),
                lang
            )
        }
        composable("my_space") {
        ...
        composable("subject_space/{subjectName}") { entry ->
            SubjectSpaceScreen(
                navController,
                mySpaceManager,
                Uri.decode(entry.arguments?.getString("subjectName") ?: ""),
                lang
            )
        }
